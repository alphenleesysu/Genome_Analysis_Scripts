...
while(<$IN>){
  ...
  my $faa_seq12=$faa_db->seq($gene12);
...
# alter num: add ">$geneXXX\n$faa_seqXXX\n"
  ...
  my $fna_seq12=$fna_db->seq($gene12);
...
#  system("cat ./tmp.ng/linsi_tmp.faa >> ./tmp.ng/tmp_check_linsi_aln.faa");
#  system("echo >>./tmp.ng/tmp_check_linsi_aln.faa "); 

# pal2nal muscle_tmp.faa to pal2nal_tmp.fna (maybe the -nomismatch setting can avoid the ambiguity of stop codon and so on?)
  system("~/software/pal2nal.v14/pal2nal.pl  ./tmp.ng/linsi_tmp.faa  ./tmp.ng/tmp.fna  -nomismatch -output fasta >./tmp.ng/pal2nal_tmp.fna");

# Gblocks --cut gaps
  system("Gblocks tmp.ng/pal2nal_tmp.fna -t=c -b5=h 1>nohup.out ");

#parse multiple-line pal2nal_tmp.fna-gb to single-line pal2nal_tmp.fna1
  system("perl -pe 's/ //g' tmp.ng/pal2nal_tmp.fna-gb |awk '/^>/&&NR>1{print \"\"} {printf \"%s\",/^>/?\$0\"\\n\":\$0}' > tmp.ng/pal2nal_tmp.fna1 ");
